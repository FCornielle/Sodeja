...
| 12 | Regulation Verifier | MVP (narrow) | Non-exhaustive checklist, 2 municipalities + national permits; no "compliant" affordance. **Citation corrected again (B-18): land use is governed by Ley 368-22 (Art. 24 Párrafo I), NOT by MOPC R-007/Decreto 284-91** — R-007 is the "Reglamento para Proyectar sin Barreras Arquitectónicas" (accessibility) and is unrelated to land use, so the earlier correction to R-007 was itself wrong. Content seeded by `packages/db/migrations/1785560000000_seed-permits-content.sql`, whose header records what was verified and what was deliberately left uncovered. formalizate.gob.do confirmed to cover only business-formation steps, not use-of-land/health/fire/environmental permits — the gap this module addresses is real. |
