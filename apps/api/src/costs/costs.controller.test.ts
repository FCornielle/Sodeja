...
describe.skipIf(!process.env.DATABASE_URL)("costs: fit-out (B-14) (DB-backed)", () => {
  ...
  async function startApp(): Promise<INestApplication> {
    const moduleRef = await Test.createTestingModule({ imports: [ProjectsModule, CostsModule] }).compile();
